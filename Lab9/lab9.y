...
*
* Recent changes made:
* 2022-11-8: include "string.h" header statement 
* 2022-11-16: include function for creating labels 
* 2022-11-16: create temporary variable for labels 
* 2022-11-16: create second temporar label varaible for second part of loops and if/elst stmts
*/
...
struct ASTnodetype *PROGRAM; // global variable declaration for program
...
FullMethodCallParmList  : MethodArg { $$ = $1; }
                           
                        | MethodArg ',' MethodCallParmList
                           {
				$$ = $1;
				$$ -> next = $3; // FullMethodCallParmList  
		           }
                        ;
...
MethodArg : Expr 
		...
		{ 
			...
			$$ -> S1 = ASTCreateNode(A_STRINGCONSTANT);
			$$ -> S1 -> name = $1;
	       		$$ -> S1 -> A_Declared_Type = A_Decaf_STRING;
	       		$$ -> S1 -> label = create_temp_label();	
	    	}
          ;
...
IfStmt : T_IF '(' Expr ')' Block
	    ...
	    $$ -> label = create_temp_label();
	    //$$ -> label2 = create_temp_label();	

	    $$ -> S2 -> S1 = $5; // block for if statement 
	    $$ -> S2 -> S2 = NULL; // when there is else block present 
	  }
...
int main(int argc, char * argv[1])
{
	...
	while (i < argc)
	{
		...
		if (strcmp(argv[i], "-o") == 0) // we have a input file
		{
			// copy argv into temporary 
			// add in suffix
...
yyparse();
	...
	if (mydebug) Display();
	if (mydebug) printf("Parsing completed\n");
	// we know that PROGRAM points to our AST
	
	if (mydebug) ASTprint(0, PROGRAM);
	EMIT(PROGRAM, fp); // call EMIT function 	
